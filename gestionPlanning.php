...
if($connect)
{
    ...
    $sql3 ="SELECT MAX(Id_planning) FROM planning_coach";
    $res3=  mysqli_query($db_handle, $sql3); 
    while ($row_3 =  mysqli_fetch_assoc($res3))
    {
        echo "wesh";
        $arrayID = $row_3['Id_planning'];
        echo $arrayID;
    }  
    ...
    for($i=0;$i<$time_slot;$i++)
    {
       ...
       $tamponFin = date('H:i',$h4+$h3);
        $sql2 = "INSERT INTO time_slot(Id_planning,Heure_debut,heure_fin,Status) VALUES (".$arrayID.",'".$tamponDebut."','".$tamponFin."', 'Actif')";
